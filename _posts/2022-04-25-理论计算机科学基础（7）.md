...
则$L_1\in \Pi_1$。因此$\overline{L_1}\in \Sigma_1$。由封闭性，存在映射归约$\overline{L_1}\leq_m A_{TM}\ via\ f$。
...
这样，只要输入$x\in L$，就存在$y_1$，使得$\langle x,y_1\rangle\in L_1(\notin \overline{L_1})$，这样M结果就是接受，即$x\in L(M)$。

因此，这里的$L$相对于$A^{TM}$就是可判定的了。又由于M接受的语言是图灵可识别的（接受 or 不停机），属于$\Sigma_1$类，因此$\Sigma_2\subseteq \Sigma_1^{A_{TM}}$。
