...
$\exist M,\forall x\in \Sigma^*$，若$x\in L$，则M接受x；若$x\notin L$，则M拒绝x。
...
$\Leftarrow$：因L图灵可识别，故$\exist M_1,\forall x\in \Sigma^*$，若$x\in L$，则$M_1$接受x；因L补图灵可识别（引理），故$\exist M_2,\forall x\in \Sigma^*$，若$x\notin L$，则$M_2$拒绝x。
...
$<M>$为图灵机M的编码，对于任意字符串$x\in \Sigma^*$，它解码后可能是一台图灵机，也可能不是，我们<u>约定</u>解码后不是一台图灵机的字符串都对应着某个特殊的图灵机$<M_0'>$，这样我们就有：全体图灵机与全体字符串对应[^2]。

[^2]: 不是一一对应，不过这不影响问题。

由集合论知识可知，$\mid \Sigma^*\mid =\alef_0$，因此全体图灵机是可枚举的。故**全体可判定语言是可枚举的**。
...
由集合论知识可知，$\mathcal{P}(\Sigma^*)=\alef_1$，因此**全体语言是不可枚举的**。故存在众多不可判定语言。
...
$$
A_{TM}=\left\{<M,X>\mid 图灵机M接受x\right\}
$$
...
1. 若M接受x，则$M_1$接受$<M,x>$；
2. 若M拒绝x，则$M_1$拒绝$<M,x>$；
...
$$
<M,x>\in A_{TM}\Rightarrow H接受<M,x>
$$

$$
<M,x>\notin A_{TM}\Rightarrow H拒绝<M,x>
$$
...
$$
\begin{aligned}
    D接受<D> & \stackrel{D定义}{\Longleftrightarrow} H拒绝<D,<D>>\\
             & \stackrel{H定义}{\Longleftrightarrow} <D,<D>>\notin A_{TM}\\
             & \stackrel{A_{TM}定义}{\Longleftrightarrow} D不接受<D>\\
\end{aligned}
$$
...
$$
L=\left\{x\mid \exists y, <x,y>\in C\right\}
$$
...
$$
\begin{aligned}
    & x\in L\Rightarrow\exist y\in \Sigma^*, <x,y>\in C\Rightarrow M接受x\\
    & x\notin L\Rightarrow\forall y\in \Sigma^*, <x,y>\notin C\Rightarrow M在x上不停机
\end{aligned}
$$
...
若M接受x，有M在x上的接受计算$c_0,c_1,c_2,...,c_m$，令$y=<c_0,c_1,c_2,...,c_m>$，那么定义

$$
C=\left\{<x,y>\mid y是M在x上的接受计算\right\}
$$

则L可写为克林尼范式：

$$
L=\left\{x\mid \exists y(=<c_0,c_1,c_2,...,c_m>),<x,y>\in C\right\}
$$■
...
$$
\begin{aligned}
    E_{TM} & =\left\{<M>\mid L(M)=\phi\right\}\\
           & =\left\{<M>\mid \forall x\forall y, y不是M在x上的接受计算\right\}\\
           & =\left\{<M>\mid \forall x, M不接受x\right\}
\end{aligned}
$$
...
$$
\begin{aligned}
    All_{TM} & =\left\{<M>\mid L(M)=\Sigma^*\right\}\\
             & =\left\{<M>\mid \forall x, \exist y, y是M在x上的接受计算\right\}
\end{aligned}
$$
